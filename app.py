from flask import Flask, request, jsonify, send_file
from flask_cors import CORS

app = Flask(__name__)
CORS(app)


@app.route("/")
def home():
    return send_file("index.html")


@app.route("/chat", methods=["POST"])
def chat():
    data = request.get_json()
    user_message = data.get("message", "").lower().strip()

    def match(keywords):
        return any(word in user_message for word in keywords)

    if match(["open", "hours", "time", "today", "tomorrow", "sunday"]):
        reply = "We are open Monday to Saturday from 10 AM to 8 PM."

    elif match(["price", "cost", "how much", "haircut", "beard", "fade", "trim"]):
        reply = "Haircut is €20, beard trim is €10, and haircut + beard is €25."

    elif match(["appointment", "book", "booking", "reserve", "walk in"]):
        reply = "You can walk in anytime, but booking via WhatsApp is recommended."

    elif match(["number", "phone", "contact", "whatsapp"]):
        reply = "Reach us on WhatsApp at +49 123 456789."

    elif match(["location", "where", "address", "near", "located"]):
        reply = "We are located in Berlin, near Alexanderplatz."

    elif match(["services", "offer", "do you do"]):
        reply = "We offer haircuts, beard trims, and a haircut + beard combo."

    elif match(["hello", "hi", "hey", "yo"]):
        reply = "Hey! How can I help you today? You can ask about our hours, prices, services, bookings, or location."

    else:
        reply = "I'm not sure about that. Feel free to ask about our hours, prices, services, bookings, or location."

    return jsonify({"reply": reply})


if __name__ == "__main__":
    app.run(debug=True)
